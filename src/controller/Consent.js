...
const Consent = {
	...
	async create(req, res) {
		...
		const createQuery = `INSERT INTO
			...
			returning *`;
		try {
			const { rows } = await db.query(findOneQuery, [req.user.user_id]);
			if (!rows[0]) {
				const values = [
					uuidv4(),
					req.user.user_id,
	...
	async getOne(req, res) {
		...
		try {
			const { rows } = await db.query(findOneQuery, [req.params.consent_id, req.user.user_id]);
			if (!rows[0]) {
	...
	async update(req, res) {
		...
		try {
			const { rows } = await db.query(findOneQuery, [req.body.consent_id, req.user.user_id]);
			if (!rows[0]) {
				return res.status(404).send({'message':'consent not found'});
			}
			const values = [
	...
	async delete(req, res) {
		...
		try {
			const { rows } = await db.query(deleteQuery, [req.params.consent_id, req.user.user_id]);
			if (!rows[0]) {
				return res.status(404).send({'message':'consent not found'});
			}
